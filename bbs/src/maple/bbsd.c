...
static void
    logattempt(type, content) int type; /* '-' login failure   ' ' success */
char *content;
{
...
static int /* 1:在list中 0:不在list中 */
    belong_list(filelist, key, desc) char *filelist,
    *key, *desc;
{
  FILE *fp;
  char buf[80], *str;
...
static int /* 回傳 multi */
    login_user(content) char *content;
{
  int attempts; /* 嘗試幾次錯誤 */
  int multi;
...
static void
    start_daemon(port) int port; /* Thor.981206: 取 0 代表 *沒有參數* , -1 代表 -i (inetd) */
{
  int n;
  struct linger ld;
